...
class QFunction(Feedforward):
    def __init__(self, observation_dim, action_dim, hidden_sizes=[100,100], learning_rate = 0.0002):
        ...
        output_size = 1  # Result is a scalar value
        super().__init__(input_size=input_size, hidden_sizes=hidden_sizes, output_size=output_size)
    ...
    def Q_value(self, observations, actions):
        ...
        ##########
        # Concatenate observations and actions along the last dimension
        combined_input = torch.cat([observations, actions], dim=-1)

        # Use Feedforward's forward method to get Q-value
        q_value = self(combined_input)  # This calls Feedforward's forward method
        return q_value
        ##########
    ...
    def train(self, iter_fit=32):
        ...
        for i in range(iter_fit):
            ...
            with torch.no_grad():
                ...
                q_target_next = self.Q_target(torch.cat((s_prime, next_action), dim=1))
...
def main():
    ...
    optParser.add_option('-m', '--maxepisodes',action='store',  type='float',
                         dest='max_episodes',default=6000,
